...
MC_Indist::MC_Indist(double _lambda, double _p, const MobMatrix& M)
:lambda{_lambda}, p{_p}{
    PROFILE_FUNCTION();
    
    Est.resize(M.Pob); Org.resize(M.Pob); Desplazamiento.resize(M.Pob);
    ...
    int k = 0;
    for(int i = 0; i < M.N; i++){
        for(int j = 0; j < M.population[i]; j++){
            Org[k] = i;
            k++;
        }
    }    
...
void MC_Indist::inicializar(double _rhoInicial){
    PROFILE_FUNCTION();
...
void MC_Indist::desplaza(const MobMatrix& T){
    PROFILE_FUNCTION();

    std::random_device rd;
    std::mt19937 mt(rd());
    std::uniform_real_distribution<double> dist(0.0, 1.0);
    for(int k = 0; k < Desplazamiento.size(); k++){
        if (dist(mt) < p){		                //SI se desplaza
            int n = 0;
            double ran = dist(mt);
            double parControl = T.Mpesos[Org[k]][0];

            while(ran > parControl && n < T.vecinos[Org[k]] - 1)
                parControl += T.Mpesos[Org[k]][++n];

            Desplazamiento[k] = T.Mvecinos[Org[k]][n];
