...
enum Estado {S, E, A, P, I, D, R};
class MC_Indist{          //Montecarlo para individuos Distinguibles -> base
...
public:
    ...
    MC_Indist(double _lambda, double _p, const MobMatrix& M);
    ...
    virtual void desplaza(const MobMatrix& T);

    const std::vector<Estado>& getEst() const {return Est;}
    const std::vector<int>& getOrg() const {return Org;}
    //const std::vector<int>& getDes() const {return Des;}  //Los indistinguibles no tienen destino propio
...
protected:
    ...
    std::vector<int> Org, Desplazamiento, InfectadosDes, InfectadosOrg, n_eff;
