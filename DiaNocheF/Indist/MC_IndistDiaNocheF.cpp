...
MC_IndistDiaNocheF::MC_IndistDiaNocheF(double _lambda, double _p, const MobMatrix& M)
: MC_IndistDiaNocheF::MC_Indist{_lambda,_p,M}
{
    PROFILE_FUNCTION();
...
void MC_IndistDiaNocheF::update(const MobMatrix& T){
    PROFILE_FUNCTION();
...
void MC_IndistDiaNocheF::iteracion(const MobMatrix& T){
    PROFILE_FUNCTION();
...
void MC_IndistDiaNocheF::calculaLambda0(const MobMatrix& T){
    PROFILE_FUNCTION();
...
double MC_IndistDiaNocheF::f(int i, double _p, const MobMatrix& T) const{

    double result = 0;  //Calcular la poblacion efectiva neff
    for(int j = 0; j < T.vecinosT[i]; j++){
        result += T.MpesosT[i][j] * T.population[T.vecinosT[j]] * _p;
    }
    
    result += T.population[i] * (1 - _p);
...
double MC_IndistDiaNocheF::f(int i, const MobMatrix& T) const{
    if(T.area[i] != 0)
        return n_eff[i]/T.area[i];
}

void MC_IndistDiaNocheF::generafvector(const MobMatrix& T){
    PROFILE_FUNCTION();
...
void MC_IndistDiaNocheF::calculateZs(const MobMatrix& T){
    PROFILE_FUNCTION();
