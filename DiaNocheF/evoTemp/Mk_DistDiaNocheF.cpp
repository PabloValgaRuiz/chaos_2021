...
void Mk_DistDiaNocheF::inicializar(std::vector<double> _rhoInicial){
    for(int i = 0; i < rhoE.size(); i++)
        for(int j = 0; j < rhoE[i].size(); j++)
            rhoE[i][j] = _rhoInicial[i];
}
...
void Mk_DistDiaNocheF::contarInfectados(const MobMatrix& M){
    ...
    for(int i = 0; i < M.N; i++){
        patchRhoE[i] = 0;
        for(int j = 0; j < M.vecinos[i]; j++){
            patchRhoE[i] += M.Mpesos[i][j] * rhoE[i][j];
