...
InstrumentationTimer* timer = new InstrumentationTimer("Matrix construction");
	...
	std::cout << T.N << "\t" << T.Links << std::endl;
	std::cout << "MegaBytes para almacenar:\t" << 4 * (unsigned long long)T.N * (unsigned long long)T.Links * sizeof(Eigen::Triplet<double>)/1000000 << "\n";
	tripletList.reserve(static_cast<int>((unsigned long long)T.N * (unsigned long long)T.Links));
...
delete timer;
	//==============================EIGEN CALCULATION===================================================//
	std::cout << "Construction ended. Beginning eigensolver." << std::endl;
