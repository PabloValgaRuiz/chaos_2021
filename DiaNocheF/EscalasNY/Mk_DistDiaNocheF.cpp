...
void Mk_DistDiaNocheF::calc_BetaIeffN(const MobMatrix& T){
    ...
    for(int i = 0; i < T.N; i++){
        I_effN[i] = 0;
        for(int j = 0; j < T.vecinos[i]; j++){
            I_effN[i] += T.Mpesos[i][j] * rhoE[i][j];
        }
    }
    #endif
    #ifdef SIR
...
double Mk_DistDiaNocheF::f(int i, const MobMatrix& T) const{
    ...
    if(T.area[i] != 0)
        return 2 - pow(2.71828, -0.001*n_eff[i]/T.area[i]);//n_eff[i]/T.area[i];
    else return 0;
}
double Mk_DistDiaNocheF::f(int i, double _p, const MobMatrix& T) const{
    ...
    if (T.area[i] != 0){
        if(_p == 0){
            return 2 - pow(2.71828, -0.001*T.population[i]/T.area[i]);//T.population[i]/T.area[i];
        }
        else{
            double tempNeff = 0;
            for(int j = 0; j < T.vecinos[i]; j++){
            ...
            return 2 - pow(2.71828, -0.001*tempNeff/T.area[i]);//tempNeff/T.area[i];
        }
    }
    else return 0;
}
